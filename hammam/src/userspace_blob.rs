...
/// ELF64 executable that first tests a bad syscall with NULL pointer,
/// then writes "hello from userspace\n" via syscall, then exits.
...
pub const HELLO_ELF: &[u8] = &[
    ...
    // Program header (56 bytes): PT_LOAD, file range [0x78 .. 0x78+0x4A)
    ...
    // Code (53 bytes at offset 0x78, loaded at 0x400000)
    // Bad call: sys_write(1, NULL, 10) → kernel should return -14, no crash
    0xb8, 0x01, 0x00, 0x00, 0x00,  // mov eax, 1          (sys_write)
    0xbf, 0x01, 0x00, 0x00, 0x00,  // mov edi, 1          (fd=stdout)
    0x31, 0xf6,                    // xor esi, esi        (buf = NULL)
    0xba, 0x0a, 0x00, 0x00, 0x00,  // mov edx, 10         (len = 10)
    0x0f, 0x05,                    // syscall
    // Good call: sys_write(1, "hello from userspace\n", 21)
    0xb8, 0x01, 0x00, 0x00, 0x00,  // mov eax, 1          (sys_write)
    0xbf, 0x01, 0x00, 0x00, 0x00,  // mov edi, 1          (fd=stdout)
