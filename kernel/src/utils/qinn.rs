...
fn draw_status(editor: &Editor, saved: bool) {
    ...
    crate::shell::print_usize(editor.cursor_row + 1);
    vga::print_colored(" Col:", 0x08);
    crate::shell::print_usize(editor.cursor_col + 1);
    if saved {
...
fn print_line_num(n: usize) {
    ...
    if n < 10 {
        vga::print_colored("  ", 0x08);
        let mut buf = [b'0' + n as u8];
        vga::print_colored(core::str::from_utf8(&buf).unwrap_or("?"), 0x08);
    } else if n < 100 {
        vga::print_colored(" ", 0x08);
        let mut buf = [b'0' + (n / 10) as u8, b'0' + (n % 10) as u8];
        vga::print_colored(core::str::from_utf8(&buf).unwrap_or("??"), 0x08);
    } else {
        let mut buf = [
            b'0' + (n / 100) as u8,
