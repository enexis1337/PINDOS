...
use crate::vga;
use crate::linux::elf::{self, LoadedElf};
use crate::linux::syscall::{SyscallRegs, ProcessState};
...
unsafe extern "C" fn linux_syscall_dispatch(regs: *mut SyscallRegs) {
    if let Some(ref mut state) = PROC_STATE {
        ...
        if state.exited {
            // Процесс завершился — восстанавливаем ядро и возвращаемся
            let code = state.exit_code;
            PageDir::restore_kernel();
            // Восстанавливаем kernel сегменты
            core::arch::asm!(
                "mov ax, 0x10",
                "mov ds, ax",
                "mov es, ax",
            ...
            // Возвращаемся из run() через longjmp-подобный механизм
            // Упрощённо: просто зависаем с кодом (в реальной ОС — планировщик)
            PROC_EXIT_CODE = code;
            PROC_EXITED = true;
...
fn setup_int80_handler() {
    ...
    unsafe {
        // IDT находится по адресу из IDTR
        let mut idtr = [0u32; 2];
