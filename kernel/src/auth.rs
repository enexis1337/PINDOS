...
pub fn read_password() -> crate::shell::InputBuf {
    let mut buf = crate::shell::InputBuf::new();
    loop {
        let c = vga::read_char();
        match c {
            b'\n' => { vga::put_char(b'\n'); break; }
            b'\x08' => {
                if buf.len > 0 {
                    buf.len -= 1;
                    // Не показываем символы, но стираем '*'
                    vga::put_char(b'\x08');
                }
            }
            _ => {
                if buf.len < crate::shell::MAX_INPUT - 1 {
                    buf.push(c);
                    vga::put_char(b'*'); // маскируем
                }
            }
        }
    }
